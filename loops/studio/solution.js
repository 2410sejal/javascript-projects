...
function mealAssembly(protein, grains, veggies, beverages, desserts, numMeals) {
  ...
  for (let i = 0 ; i < numMeals; i++){  
    ...
    console.log("Pantry Length:",pantry.length);
    for (let j = 0; j < pantry.length; j++){
      console.log("Iteration:",j);
      console.log("Pantry Array",pantry[j]);
      console.log("Pantry Item",pantry[j][i]);
      meal.push(pantry[j][i]);
      console.log("Meal Prepared",meal,"for person",i);
    }
    meals.push(meal);
  }
  console.log("Total Meals Prepared",meals.length);
  return meals;
}
